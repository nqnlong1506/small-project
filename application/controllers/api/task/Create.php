 ...
 */
class Create extends \Restserver\Libraries\REST_Controller {
    ...
    public function create_task_post()
    {
        ...
        if ($title == null || $title == '') 
        {
            $message = json_response(false, 'title of task required.');
        ...
        $task_id = $this->Task_model->insert_task($task_data);

        if ($task_id == 0) 
        {
            $message = json_response(false, 'isnert task failed.');
            $this->response($message, \Restserver\Libraries\REST_Controller::HTTP_INTERNAL_SERVER_ERROR);

            return;
        }

        $message = json_response(true, 'insert task successfully.', $task_id);
